...
class AudioManager {
  static late AudioManager _instance;
  ...
  Future<String> startInfo(AudioInfo audio, {required bool auto}) async {
    return await start(audio.url, audio.title, desc: audio.desc, cover: audio.coverUrl);
  }
  ...
  Future<String> previous() async {
    if (playMode != PlayMode.single) {
      ...
      if (index < 0) {
        ...
        _curIndex = index.toInt();
      }
    }
    return await play(index: _curIndex);
  }
